# AICE™ — Adaptive Institutional Collaborative Engine™
### LOTTO-AICE™ • LAST OF TRUE TORONTO, LTD.™

AICE™ is the world’s first **Institutional Intelligence Partner** — an agent designed not to automate tasks, but to transform founder cognition into structured institutional outputs.

This project is built for the **Google Cloud Agentic Hackathon (Collaborative Partner Track)**.

---

## 🚀 What AICE™ Does
AICE™ takes unstructured founder input — notes, ideas, voice memos, half‑formed plans — and converts it into:

- Structured plans  
- Decision frameworks  
- Institutional doctrine  
- Submission artifacts  
- Memory‑linked continuity  

AICE™ is not a chatbot.  
AICE™ is a **Collaborative Partner**.

---

## 🧠 Core Architecture (MVP)
AICE™ runs as a single agent with:

- **Reasoning Loop**  
- **Tool Router**  
- **NotesStore** (memory)  
- **TaskPlanner** (structure)  
- **DocsTemplate** (artifacts)  
- **Cloud Run Deployment**

This repository contains the full MVP scaffolding.

---
/aice
/agent
aice_agent.py
reasoning_loop.py
tools_router.py
/tools
notes_store.py
task_planner.py
docs_template.py
/memory
session_memory.py
project_memory.py
/api
chat.py
Dockerfile
requirements.txt

---

## 🎯 Hackathon Goal
Deliver a working Collaborative Partner agent that:

- Accepts founder input  
- Uses tools to structure information  
- Writes memory  
- Returns clean institutional artifacts  
- Runs live on Cloud Run  
- Demonstrates agentic reasoning  

---

## 📹 Demo (to be added)
A short video showing AICE™ converting personal messy notes + research into a structured submission plan.

---

## 🏢 About LAST OF TRUE TORONTO, LTD.
A world‑first institutional design company building myth‑tech systems, adaptive intelligence engines, and founder‑grade operational frameworks.

AICE™ is its first public agent.



## 📁 Repository Structure

